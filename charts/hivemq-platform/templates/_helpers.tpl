...
{{- include "hivemq-platform.validate-trustore" . }}
...
{{/*
Validates trustore configuration for the secured services:
 - Trustore mandatory value `truststoreSecretName` is present when configuring mTLS.
 - Trustore-related values can only be set for MQTT or WebSocket services.
 - Trustore-related values can only be set if keystoreSecretName is also present.
 - Trustore password is present, either in the .truststorePassword or .truststorePasswordSecretName values.
 - Trustore password is not set in both .truststorePassword and .truststorePasswordSecretName values.
Usage: {{ include "hivemq-platform.validate-trustore" . }}
*/}}
{{- define "hivemq-platform.validate-trustore" -}}
{{- $services := .Values.services }}
{{- range $service := $services }}
