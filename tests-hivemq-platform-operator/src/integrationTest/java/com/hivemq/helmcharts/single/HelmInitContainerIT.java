...
class HelmInitContainerIT extends AbstractHelmChartIT {
    ...
    void withAdditionalInitContainer_hivemqRunningWithVolumeMounts() throws Exception {
        ...
        installChartsAndWaitForPlatformRunning(
                additionalVolumeFile, "config.overrideInitContainers=" + additionalInitContainerFile);
    ...
    private void assertThatFileContains(final @NotNull Pod pod) {
        final String fileStr = "/init-container-test/init-container-test.txt";
        ...
        try (InputStream in = client.pods()
            ...
            assertThat(foundContent).contains(expectedContent);
